...
class AffiliateController extends Controller
{
    ...
    /**
     * Affiliate link üzerinden yönlendirme işlemi
     *
    ...
    {
        $affiliate = Affiliate::where('affiliate_code', $code)->firstOrFail();


        // Affiliate link aktif değilse ana sayfaya yönlendir
        if (!$affiliate) {
            return redirect('/');
        }

        // Affiliate tıklamasını kaydet
        $click = $this->trackingService->trackClick($request, $affiliate->id);

        // Affiliate cookie'sini ayarla (30 gün)
        Cookie::queue('affiliate_id', $affiliate->id, 60 * 24 * 30);
        Cookie::queue('affiliate_session', $request->session()->getId(), 60 * 24 * 30);

        // Hedef URL'ye yönlendir
        $targetUrl = $affiliate->target_url ?? '/';

        // UTM parametrelerini aktar
        $utmParams = [];
        foreach (['utm_source', 'utm_medium', 'utm_campaign', 'utm_term', 'utm_content'] as $param) {
            if ($request->has($param)) {
                $utmParams[$param] = $request->query($param);
            }
        }

        // Zaten bir UTM yoksa affiliate'i source olarak ekle
        if (!isset($utmParams['utm_source'])) {
            $utmParams['utm_source'] = 'affiliate';
            $utmParams['utm_medium'] = 'referral';
            $utmParams['utm_campaign'] = $affiliate->code;
        }

        if (count($utmParams) > 0) {
            $targetUrl .= (strpos($targetUrl, '?') !== false ? '&' : '?') . http_build_query($utmParams);
        }
     ...
     */
    public function markConversion(Request $request, $conversionId = null)
    {
        if (Cookie::has('affiliate_session')) {
            $sessionId = Cookie::get('affiliate_session');
            $this->trackingService->markAsConverted($sessionId, $conversionId);

            return response()->json(['success' => true]);
        }

        return response()->json(['success' => false, 'message' => 'No affiliate session found']);
    }
    /**
