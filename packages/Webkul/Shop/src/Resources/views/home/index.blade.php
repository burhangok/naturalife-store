    ...
    @if ($allProducts && $allProducts->count())
        <section class="container mt-20 max-lg:px-8 max-md:mt-8 max-sm:mt-7 max-sm:!px-4">
            <h2 class="font-dmserif text-3xl max-md:text-2xl max-sm:text-xl"
                style="margin-bottom: 50px; margin-top: 50px">Alle Produkte</h2>

            <div
                class="grid grid-cols-3 gap-8 max-[1060px]:grid-cols-2 max-md:grid-cols-2 max-sm:grid-cols-2 max-md:gap-4 max-md:justify-items-center">

                @foreach ($allProducts as $product)
                    <div
                        class="1180:transition-all group w-full max-w-[320px] rounded-md border border-transparent p-2.5 hover:border-gray-300 mx-auto">

                        <!-- Product Image Container -->
                        <div
                            class="relative max-h-[300px] w-full overflow-hidden max-md:max-h-60 max-md:max-w-full max-md:rounded-lg max-sm:max-h-[200px] max-sm:max-w-full">

                            <!-- Product Image -->
                            <a href="{{ url($product->url_key) }}" aria-label="{{ $product->name }}">
                                <img src="{{ $product->base_image_url ?: asset('themes/shop/default/assets/images/product-placeholders/front.png') }}"
                                    alt="{{ $product->name }}"
                        ...
                        <div
                            class="-mt-9 grid w-full translate-y-9 content-start gap-2.5 bg-white p-2.5 transition-transform duration-300 ease-out group-hover:-translate-y-0 group-hover:rounded-t-lg max-md:relative max-md:mt-0 max-md:translate-y-0 max-md:gap-0 max-md:px-0 max-md:py-1.5">

                            <!-- Product Name -->
                            <p
                                class="break-all text-base font-medium max-md:mb-1.5 max-md:whitespace-break-spaces max-md:leading-6 max-sm:text-sm max-sm:leading-4">
                                {{ $product->name }}
                            </p>
                            ...
                            <!-- Add to Cart Button -->
                            <div
                                class="action-items flex items-center justify-between gap-2 opacity-0 transition-all duration-300 ease-in-out group-hover:opacity-100 max-md:opacity-100">
