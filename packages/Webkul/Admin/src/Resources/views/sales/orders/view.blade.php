            ...
            <div class="card mt-4">
                ...
                <div class="card-body">
                    <div class="alert alert-info row">
                       ...
                       @endphp
                            <strong>Toplam Komisyon Oranı:</strong> {{ round($commissionRate, 2) . "%" }} </div>
                    </div>
...
Kupon Satış Komisyonu
                                ...
                                <div class="col-md-6">
                                    <dl class="row">
                                        ...
                                        <dd class="col-sm-8">
                                            ...
                                            <a href="{{ route('admin.affiliatemodule.admin.affiliates.edit', $commission->from_affiliate_id) }}">
                                                #{{ $commission->from_affiliate_id }} -
                                                {{ $commission->fromAffiliate->customer->getNameAttribute() ?? $commission->fromAffiliate->email }}
                                            </a>
                                            @elseif($commission->level == 1)
                                        ...
                                        <dd class="col-sm-8">{{ $commission->created_at->format('d.m.Y H:i') }}</dd>
                                    </dl>
                                </div>
                            </div>
                        ...
                        {{$commission->description}}
                            ...
                            <div class="mt-3">
                                <h6 class="font-weight-bold">Komisyon Açıklaması:</h6>
                                <div class="p-3 bg-light rounded">
                                    Sipariş #{{ $commission->order_id }} için {{ $commission->level }}. seviye komisyon (Sepet Tutarı: {{ core()->formatBasePrice($order->base_grand_total - $order->shipping_amount) }} Euro)
                                </div>
                            </div>
                            @endif
...
Kupon ile yapılan alışverişten kazanılan komisyon.
                        ...
                        @if($commission->level == 1)
                                    ...
                                    @else
                                        Bu komisyon, {{ $commission->level }}. seviye temsilci ilişkisinden kaynaklanmaktadır.
                                        {{ $commission->fromAffiliate ? '#'.$commission->from_affiliate_id.' - '.$commission->fromAffiliate->name ?? $commission->fromAffiliate->email : 'Belirtilmemiş' }}
